...
int main(){
    ...
    unsigned int M_cache = (40 * m_files / 100);    //dimensione della cache 10% del catalogo
    ...
    for(unsigned int i=(n_utenti); i<n_utenti; i++)
    ...
    for(unsigned int k=0; k<L_request; k++)
    {
        ...
        for (int i = 0; i < (n_utenti); i++){
            for (int j = 0; j < (memory_per_user.at(i)*2+3); j++){
                cout << input.at((memory_per_user.at(i)*2+3)*i+j) << " ";
            }
            cout << endl << endl;
        }
